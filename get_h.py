...
from properties import air_prop, kf


def get_h(t_f, t_a, t_g, t_ab, t_i, nodes, t_am, delta_a, d_in, k, w_f):
    ...
    h_r1 = np.zeros(nodes)
    h_c1 = np.zeros(nodes)
    h_g_am = np.zeros(nodes)
    h_i_am = np.zeros(nodes)
    Nu_f = np.zeros(nodes)
    Nu_a = np.zeros(nodes)
    Ra = np.zeros(nodes)
    ny_am = 1.5743 * 10**-5
    ...
    for j in range(nodes):
        ...
        if AA <= 0:
            ...
            Nu_a[j] = 1 + (1.44 * (1 - (1708 * (np.sin(1.8 * theta))**1.6 / (Ra[j] * np.cos(theta)))) * AA)
        else:
            Nu_a[j] = 1 + (1.44 * (1 - (1708 * (np.sin(1.8 * theta))**1.6 / (Ra[j] * np.cos(theta)))) * AA) + BB
